...
const CalculateSmaSignals = (
  ...
  for (let i = 1; i < stockDataPrices.x.length; i++) {
    const index = stockDataIndicator.x.findIndex(
      (date) => date === stockDataPrices.x[i]
    );
    if (index !== -1) {
      if (
        stockDataPrices.y[i - 1] < stockDataIndicator.y[index + 1] &&
        stockDataPrices.y[i] > stockDataIndicator.y[index]
      ) {
...
export default CalculateSmaSignals;
